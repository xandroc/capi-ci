...
pushd capi-checkman
  ...
  for pipeline in $pipelines; do
    curl -s "https://capi.ci.cf-app.com/api/v1/pipelines/$pipeline/jobs" | jq .[].name --raw-output | awk -v pipeline="$pipeline" -v atc_url="$atc_url" '{print $1 ": concourse.check " atc_url " " pipeline " " $1}' >> pipeline
  done
