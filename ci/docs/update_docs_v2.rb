...
cf_deployment_versions = `git -C $PWD/cf-deployment tag -l --sort=v:refname`.split
...
versions_to_add.each do |cf_deployment_version|
  capi_version=`git -C $PWD/cf-deployment show #{cf_deployment_version}:cf-deployment.yml | grep capi-release?v= | cut -d'=' -f2`.strip
  cc_version=`git -C $PWD/capi-release ls-tree #{capi_version} src/cloud_controller_ng | awk '{print $3}'`.strip
  api_version=`git -C $PWD/capi-release/src/cloud_controller_ng show #{cc_version}:config/version_v2`.strip
