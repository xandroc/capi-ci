...
export BOSH_ENVIRONMENT BOSH_CLIENT BOSH_CLIENT_SECRET BOSH_CA_CERT JUMPBOX_URL JUMPBOX_SSH_KEY JUMPBOX_USERNAME
...
sshuttle -e "ssh -i "${JUMPBOX_PRIVATE_KEY}" -o 'StrictHostKeyChecking no' -o 'UserKnownHostsFile /dev/null'" -r "${JUMPBOX_USERNAME}@${ssh_jumpbox_url}" ${BOSH_GW_HOST} &
tunnel_pid_jumpbox="$!"
...
cleanup() {
  kill "${tunnel_pid_jumpbox}"
  kill "${tunnel_pid_director}"
  rm -rf "${tmpdir}"
}
trap 'cleanup' EXIT
